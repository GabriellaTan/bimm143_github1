...
Let's polish it, so it doesn't generate with "Quote" marks. I want it to be like "ACCTGGAATGGC" so it can be BLASTed!!!
```{r}
...
We want R to produce multiple sequences. Do this by either:
- *editing* and *adding* the function body code (for a loop), OR
- use the R **apply** family of utility fctn.
...
** MHC presents 9 amino acid sequences too. 
** BLASTp has a window of 9 — if they are similar for 9 or above, then it's probably a related protein!
